...
public class GraphTraversal {
    ...
    // =========================================================
    // DFS - ITERATIVE (pakai Stack eksplisit)
    // =========================================================
    /**
     * Versi iteratif DFS menggunakan Stack secara eksplisit.
     * Ini untuk nunjukkin kamu ngerti Stack sebagai struktur data.
     *
     * Catatan: urutan kunjungan bisa beda dengan rekursif
     * karena Stack di sini push neighbor dari kiri ke kanan.
     */
    public void dfsIterative(String startCode) {
        if (!graph.getAdjList().containsKey(startCode)) {
            System.out.println("✖ Stasiun " + startCode + " tidak ditemukan!");
            return;
        }

        System.out.println("\n===== DFS ITERATIVE dari " + startCode + " =====");

        Set<String> visited = new HashSet<>();

        // Stack adalah jantungnya DFS iteratif (LIFO = Last In First Out)
        ...
        while (!stack.isEmpty()) {
            ...
            visited.add(current);
            System.out.println("  Mengunjungi: " + graph.getStations().get(current));

            // Push semua tetangga yang belum dikunjungi ke stack
            for (Route route : graph.getConnections(current)) {
                if (!visited.contains(route.getDestination())) {
                    stack.push(route.getDestination());
                }
            }
        }
